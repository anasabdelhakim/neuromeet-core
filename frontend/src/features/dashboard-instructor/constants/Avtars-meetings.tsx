...
export const AvatarChain = ({ avatars = Avatars, max = 4 }) => {
  return (
    <div className="flex">
      ...
      {avatars.length > max && (
        <div
          className="relative w-8 h-8 rounded-full border-2 border-background bg-primary-soft-high flex items-center justify-center text-[11px] font-bold text-primary-light -ml-3 shadow-sm backdrop-blur-sm"
          style={{ zIndex: 0 }}
        >
          +{avatars.length - max}
        </div>
