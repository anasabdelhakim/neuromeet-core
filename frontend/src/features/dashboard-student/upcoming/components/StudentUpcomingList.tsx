...
export async function StudentUpcomingList() {
  ...
  if (meetings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center border border-border/50 rounded-xl bg-black-soft-subtle/30 backdrop-blur-sm shadow-inner">
        <CalendarX size={48} className="text-muted-foreground mb-4 opacity-40 animate-pulse" />
        <p className="text-muted-foreground font-medium">No upcoming meetings found.</p>
